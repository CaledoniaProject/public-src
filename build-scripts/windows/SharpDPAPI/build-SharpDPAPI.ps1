...
$build_dir = $env:temp + "\SharpDPAPI-master"
$zip_path  = $env:temp + "\SharpDPAPI-master.zip"
...
$web.DownloadFile("https://github.com/GhostPack/SharpDPAPI/archive/master.zip", $zip_path);
...
cmd /c c:\windows\nuget.exe restore SharpDPAPI.sln
...
cd "$build_dir\SharpDPAPI\bin\Release"

ForEach ($item in "SharpDPAPI.exe") {
